...
import { createNumberRange, getValidDate } from "../lib/helpers";
...
export function FilterValueDateDisplay<TData>({
  ...
  if (filter.values.length === 1) {
    ...
    const formattedDateStr = format(value ?? new Date(), "MMM d, yyyy");

    return <span>{formattedDateStr}</span>;
  }

  const from = getValidDate(filter.values[0]);
  const to = getValidDate(filter.values[1]);

  const formattedRangeStr = formatDateRange(from, to);
...
export function FilterValueDateController<TData>({
  ...
  const [date, setDate] = useState<DateRange | undefined>({
    from: getValidDate(filter?.values[0]),
