...
// ...existing code...
import { LaptopIcon, MonitorIcon, SmartphoneIcon, TabletIcon, AppleIcon, WindowsIcon, LinuxIcon } from "lucide-react";
...
// 2. Get current device OS and name
function getCurrentDeviceInfo() {
  ...
  // Device name is not directly available; fallback to platform
...
function getDeviceIcon(os: string) {
  switch (os) {
    case "Windows":
      return <WindowsIcon className="mr-2" />;
    case "macOS":
    ...
    <div>
      {deviceSessions.map((session, idx) => (
        <div key={idx} className="flex items-center gap-2 py-2">
          ...
          <span className="text-xs text-muted-foreground ml-2">{session.os}</span>
          <span className="text-xs text-muted-foreground ml-4">{session.lastLogin}</span>
        </div>
      ))}
    </div>
  );
...
// To get current device info:
const currentDevice = getCurrentDeviceInfo();
// ...existing code...import { useForm } from "react-hook-form";
import { z } from "zod";
