          ...
          <div className="border-secondary rounded-lg border p-2 sm:p-3">
            <Form {...otpForm}>
              <form onSubmit={otpForm.handleSubmit(handleOtpSubmit)}>
                <div className="flex w-full flex-col gap-3 sm:w-[80%] lg:w-[60%]">
      ...
      >
        <div className="flex w-full flex-col gap-[40px] px-2 sm:px-4">
          {showEnableOtpAuth ? (
            ...
            <>
              {/* Password Update */}
              <section className="grid grid-cols-1 max-sm:gap-5 sm:grid-cols-2">
                <div className="flex flex-col gap-1">
                  <h4 className="text-sm font-semibold sm:text-lg">
                ...
                <div className="border-secondary flex-grow gap-2 rounded-lg border px-4 py-8">
                  <div className="grid w-full grid-cols-2 items-center gap-2">
                    <fieldset>
                      ...
                      <div className="flex items-center gap-3 border border-secondary rounded-full overflow-hidden">
                        <span className="bg-secondary py-1 px-2">{getDeviceIcon(getCurrentDeviceInfo().os)}</span>
                        <span className="flex-grow flex gap-2 items-center rounded-e-full">
                         <span className="text-sm">{getCurrentDeviceInfo().os}</span>
                         <span className="text-xs font-normal">Start time{new Date(deviceSessions.find(({os, deviceName})=> os === getCurrentDeviceInfo().os || deviceName === getCurrentDeviceInfo().deviceName)?.lastLogin ?? "").toLocaleDateString({numberingSystem: ''})}</span>
                        </span>
                      </div>
                    </fieldset>
                  </div>
