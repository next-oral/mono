            ...
            <div className="flex items-center space-x-4">
              ...
              >
                {getCurrentPreviewSource(field) ? (
                  <img
                    src={getCurrentPreviewSource(field) ?? "/placeholder.svg"}
                    alt="Avatar preview"
              ...
              <div className="flex-1">
                ...
                {getCurrentPreviewSource(field) && (
                  <p className="text-foreground mt-1 text-xs">
                    {field.value ? "New file selected" : "Current avatar"}
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="flex flex-col items-center justify-center space-y-3 text-center">
                {getCurrentPreviewSource(field) && allowPreview ? (
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="flex flex-col gap-1 text-sm text-xs font-medium">
                          {/*eslint-disable-next-line @typescript-eslint/no-unnecessary-condition*/}
                      {truncateFileName(field.value.name as string, 20) ?? (typeof field.value === "object"
                        ? Object.entries(field.value).map(([_, file]) => (
                            <span>
                            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-member-access*/}
                              {truncateFileName((file as any).name as string, 15)}
                            </span>
                          ))
                        : "File(s) selected")}
                    </span>
