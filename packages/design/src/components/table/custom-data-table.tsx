...
// Base props without filter
interface BaseCustomDataTableProps {
  data: any[]
  loading?: boolean
  ...
  // REMOVED: rowIdKey - This prop has been removed as it was unused.
  // The table now uses auto-generated sequential IDs (1, 2, 3...) for all selection operations,
  // completely ignoring any ID fields from the data. This makes the table more predictable
  // and eliminates the need for rowIdKey.
...
// Props with optional filter
interface CustomDataTablePropsWithOptionalFilter extends BaseCustomDataTableProps {
  filter?: never
  filterKeys?: never
}

// Props with required filter configuration
interface CustomDataTablePropsWithFilter<T = any> extends BaseCustomDataTableProps {
  data: T[]
  // Filter Configuration - for external filtering (like Staff, Doctor, Administrator buttons)
  filter: string[] // Array of filter values to match against
  filterKeys: T extends any[]
    ? T[0] extends Record<string, any>
      ? (keyof T[0])[] // Type-safe keys from the data
      : string[]
    : string[] // Keys in the data to filter by (e.g., ['role', 'type'])
}

// Union type for the component props
export type CustomDataTableProps<T = any> = CustomDataTablePropsWithOptionalFilter | CustomDataTablePropsWithFilter<T>
...
const DefaultEmptyState = () => (
  <div className="flex flex-col items-center justify-center py-12 text-center">
    ...
    <p className="text-muted-foreground text-sm">There are no records to display at the moment.</p>
  </div>
)
...
const DefaultNoSearchResults = ({ searchTerm }: { searchTerm: string }) => (
  <div className="flex flex-col items-center justify-center py-12 text-center">
    ...
    <h3 className="text-foreground mb-2 text-lg font-medium">No results found for "{searchTerm}"</h3>
    <p className="text-muted-foreground text-sm">Search for something else.</p>
  </div>
)

// Main Component
export const CustomDataTable = <T = any>({
  data = [],
  loading = false,
  searchable = true,
  ...
  className,
  rowClassName,
  // Filter props - for external filtering (e.g., Staff, Doctor, Administrator buttons)
  filter,
  filterKeys,
            ...
            <div className="flex items-center space-x-3">
              ...
              {row.name && (
                <div>
                  ...
                  {row.phone && <div className="text-muted-foreground text-xs">{row.phone}</div>}
                </div>
              )}
            </div>
          )
  ...
  // Apply external filters first (before search and sort)
  const filteredByExternalFilter = useMemo(() => {
    // If no filter is provided or filter is empty, return all data
    if (!filter || filter.length === 0) {
      return data
    }

    // Filter data based on the provided filter values and keys
    return data.filter((row) => {
      // Check if any of the filterKeys contain any of the filter values
      return filterKeys.some((key) => {
        const value = row[key as string]
        if (value == null) return false

        // Convert to string and check if any filter value matches (case-insensitive)
        const stringValue = String(value).toLowerCase()
        return filter.some((filterValue) => stringValue.includes(filterValue.toLowerCase()))
      })
    })
  }, [data, filter, filterKeys])

  // Filter and search data (after external filtering)
  const filteredData = useMemo(() => {
    if (!searchTerm.trim()) return filteredByExternalFilter

    return filteredByExternalFilter.filter((row) =>
  ...
  }, [filteredByExternalFilter, searchTerm, tableColumns])
      ...
      return (
        <div className="space-y-1">
          ...
          {column.subFields.map((subField) => (
            <div key={subField.key} className={cn("text-muted-foreground text-sm", subField.className)}>
      ...
      <div className={cn("space-y-4", className)}>
        ...
        {searchable && (
          <div className="flex items-center justify-between">
            ...
            {showSeeAll && <div className="bg-secondary h-10 w-20 animate-pulse rounded-md" />}
          </div>
        )}
  ...
  return (
    <div className={cn("space-y-1 bg-secondary px-1", className)}>
      {/* Search and Controls */}
      {(searchable || showSeeAll || internalSelectedRows.length > 0 || hasNumberOfRows) && (
            ...
            {searchable && (
              <div className="relative max-w-sm flex-1">
                ...
                <Input
                  ...
                  className="pr-10 pl-10 sm:rounded-none focus-visible:shadow-none focus-visible:ring-0 sm:focus-visible:border-0 sm:border-0"
                />
                {/* Clear search button */}
                {searchTerm && (
                  <Button
              ...
              <TableHeader>
                <TableRow>
                  ...
                  {tableColumns.map((column) => (
                    <TableHead
                      key={column.key}
                      className={cn(column.sortable && "hover:bg-accent cursor-pointer select-none", column.className)}
