...
export function PendingDropOverlay({ visible, appointment, dentistId, pendingStartMinutes }: Props) {
    const { getAppointmentWidth, getAppointmentHeight, getAppointmentTop, getAppointmentLeft } =
        useCalendarStore();

    if (!visible || !appointment || typeof pendingStartMinutes !== "number") return <div>dvuduyhgj vdjh buhbvjbnhv jhedvbjhbjhbv d uh</div>;
    ...
    return (
        ...
        >
            <div
                className={cn("pointer-events-none flex rounded-md p-2", appointment.color?.stickerColor)}
                style={{ width: "100%", height: "100%" }}
            >
                <div className={cn("mr-2 w-1 flex-shrink-0 rounded-full mt-20", appointment.color?.lineColor)} />
                <div className="flex-1 overflow-hidden">
                    <div className="text-xs leading-tight font-medium">{appointment.patientName}</div>
                    <div className="text-xs leading-tight opacity-75">
                        {minutesToTime(pendingStartMinutes)} - {appointment.endTime /* or recompute end */}
                    </div>
                </div>
            </div>
        </div>
