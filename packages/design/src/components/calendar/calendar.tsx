...
import { PendingDropOverlay } from "./components/pending-drop-overlay";
...
function CalendarBody() {
    ...
    const targetAppointment = useCalendarStore(
        (state) => state.targetAppointment,
    );
    const setTargetAppointment = useCalendarStore(
        (state) => state.setTargetAppointment,
    );

    const showConfirmDialog = useCalendarStore(
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        function getNewEndTime(
            ...
            const originalStartMins = timeToMinutes(String(originalAppointment?.startTime));
            const originalEndMins = timeToMinutes(String(originalAppointment?.endTime));
            const newStartMins = timeToMinutes(newStartTime);

            // 2. Calculate the original duration (in minutes)
            let durationMins = originalEndMins - originalStartMins;

            // Handle cross-day duration (e.g., 23:00 to 01:00) 
            // If the original end time is numerically before the start time, 
            // it implies a cross-day appointment. Add 24 hours (1440 minutes).
            if (durationMins < 0) {
                durationMins += 24 * 60;
            }

            // 3. Calculate the new end time (in total minutes)
            const newEndMins = newStartMins + durationMins;

            // 4. Convert the new total minutes back to "HH:MM" format

            return minutesToTime(newEndMins);
        }
            ...
            return prevAppointments.map(appointment => {
                ...
                if (appointment.id === originalAppointment.id) {
                    return {
                        ...appointment,
                        startTime: newStart,
                        endTime: getNewEndTime(newStart)
                    };
                }
                return appointment;
            });
    ...
    return (
        ...
        >
            ...
            <PendingDropOverlay
                ...
                appointment={originalAppointment ?? targetAppointment ?? findActiveAppointment() ?? null}
                dentistId={Number(targetAppointment?.dentistId ?? originalAppointment?.dentistId)}
                pendingStartMinutes={pendingNewStartMinutes}
            />
