...
function CalendarBody() {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    function getNewEndTime(newStartTime: string): string {
      // 1. Convert all times to minutes from midnight
      const originalStartMins = timeToMinutes(
        String(originalAppointment?.startTime),
      );
      const originalEndMins = timeToMinutes(
        String(originalAppointment?.endTime),
      );
  ...
  return (
    ...
    >
      ...
      <PendingDropOverlay
        ...
        dentistId={Number(
          targetAppointment?.dentistId ?? originalAppointment?.dentistId,
        )}
