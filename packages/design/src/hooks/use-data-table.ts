...
import type { ExtendedColumnSort } from "@repo/design/types/data-table";
import { useDebouncedCallback } from "@repo/design/hooks/use-debounced-callback";
import { getSortingStateParser } from "@repo/design/lib/parsers";
...
export function useDataTable<TData>(props: UseDataTableProps<TData>) {
  ...
  const filterParsers = React.useMemo(() => {
    ...
    >((acc, column) => {
      if (
        column.meta?.variant === "number" ||
        column.meta?.variant === "range"
      ) {
        acc[column.id ?? ""] = createParser({
